...
class LLAMA3_1_8BQuantizerConfig(QuantizerConfig):
    pp_size: int = 1
    tp_size: int = 1

    def __init__(self,
                 *args,
                 model_name: str = "llama3_1-8b-instruct-hf",
                 model_path: os.PathLike = paths.MODEL_DIR / "Llama3.1-8B/Meta-Llama-3.1-8B-Instruct",
                 output_path: os.PathLike = paths.MODEL_DIR / "Llama3.1-8B",
                 dataset_path: os.PathLike = paths.BUILD_DIR / "preprocessed_data/llama3.1-8b/mlperf_llama3.1-8b_calibration_1k",
        ...
        if dtype_out not in ckpt_dir_map:
            raise ValueError(f"Unsupported Precision for LLAMA3.1-8B: {dtype_out.valstr}")
