...
__global__ void top_k_score_per_image_prepare_outputs(T_SCORE* input_scores, 
                                                      int* input_indices,
                                                      T_SCORE* output_scores, int* output_indices,
                                                      int *in_end_offsets, int *out_active_counts, 
                                                      int items, int segments, int num_top_k) {
    ...
    void* workspace)
{
    ...
    segmented_scan((int *)active_count, (int *)active_count, (int *)active_count_per_batch, num_images, num_classes, stream); 

    compact_segments(unsorted_scores, unsorted_bbox_indices, sorted_scores, sorted_bbox_indices ,
                    (int *)active_count, (int *)active_count_per_batch, num_classes, 
                    num_images, num_items_per_image, num_items_per_image, stream);
    ...
    if (0) {
        ...
        get_active_counts<128><<<div_up(num_images, 128), 128, 0, stream>>>
                                (reinterpret_cast<int *>(temp_active_count), reinterpret_cast<int *>(active_count_per_batch), 
                                items, num_images, num_top_k);
    } else {
