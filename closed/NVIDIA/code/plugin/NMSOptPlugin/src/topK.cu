...
__global__ void compact_segments_kernel(const int *in, const int * in_indices, 
...
void segmented_scan(int *active_count_in, int *active_count_out, 
                 int *active_count_per_batch, int num_images, int num_classes, cudaStream_t stream) {
...
void  compact_segments( void *input_output_scores, void *input_output_indices,
                      void *temp_scores, void *temp_indices,
                      int *active_count, int *active_count_per_batch,
                      int num_classes, int num_images, int num_items_per_image, int image_stride, 
                      cudaStream_t stream) {

  compact_segments_kernel<<<dim3(num_classes, num_images, 1), 128, 0, stream>>>
                      ( (int*)input_output_scores, (int*)input_output_indices,
                      (int*)temp_scores, (int*)temp_indices,
                      (int*)active_count, (int*)active_count_per_batch,
...
void top_k(top_k_kernel* kernel, int kernel_index, void* input_output_scores, void* input_output_indices, 
           void* temp_scores, void* temp_indices, int* active_count, int* active_count_per_batch,
  ...
  compact_segments(input_output_scores, input_output_indices, temp_scores, temp_indices,
                   active_count, active_count_per_batch, num_classes, 
                   num_images, num_items_per_image, image_stride, stream);
...
template <int BLOCK_THREADS>
void top_k_n_pass(top_k_kernel* top_k_kernels, void* input_output_scores, void*input_output_indices, 
           void* temp_scores, void*temp_indices, int* active_count, int* active_count_per_batch,
  ...
  //printf("top_k_n_pass: num_top_k = %d, num_classes = %d, num_images = %d, num_items_per_image = %d, image_stride = %d\n", num_top_k, num_classes, num_images, num_items_per_image, image_stride);
  ...
  top_k<BLOCK_THREADS>(top_k_kernels, kernel_index, input_output_scores, input_output_indices, 
                       temp_scores, temp_indices, 
  ...
  if (num_segments > 1) {
    ...
    if (kernel_index >= 32) {
      top_k_n_pass<BLOCK_THREADS>(top_k_kernels, input_output_scores, input_output_indices, 
                           temp_scores, temp_indices, 
                           active_count, active_count_per_batch, 
...
void top_k_multi_pass(void* input_output_scores, void* input_output_indices, 
           void* temp_scores, void* temp_indices, int* active_count, int* active_count_per_batch,
           int num_items_per_image, int image_stride, 
           int num_top_k, int num_classes, int num_images, cudaStream_t& stream) {
  ...
  top_k_kernel top_k_kernels[] = {
    ...
    top_k_n_pass<BLOCK_THREADS>(top_k_kernels, 
                       input_output_scores, input_output_indices, 
                        temp_scores, temp_indices, 
                        (int*)active_count, (int*)active_count_per_batch, 
                        num_items_per_image, image_stride, 
