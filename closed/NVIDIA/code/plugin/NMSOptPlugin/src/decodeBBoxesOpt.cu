    ...
    if(reshape_before_permute) {
        ...
        for (int i = 0;i < 4;++i) {
            ...
            // debug
            {
                //if (d == 0 && c == 0) 
                // {
                //     printf("mappedIndex = %d, loc_data_layer = %f\n", mappedIndex, loc_data_layer[mappedIndex]);
                // }
            }
        }
    }
        ...
        const DecodeBBoxesOptData<T_BBOX, NUM_LAYERS> decodeBBoxesOptData)
{
         ...
         index += nthds_per_cta * gridDim.x)
    {
        ...
        mapLocData<T_BBOX, NUM_LAYERS>(c, d, n, loc_data,
                                       ...
                                       concatInputs, box_channels, conf_channels, packed32_nchw);
        ...
        const float4 prior_data_v = (!variance_encoded_in_target)? to_float4(
            *(reinterpret_cast<const VECTOR_TYPE*>
                (prior_data + vi))) : one_float4;
        ...
        else if (code_type == CodeTypeSSD::CENTER_SIZE)
        {
            ...
            bbox_data_val = make_float4(
                                        decode_bbox_center_x - decode_bbox_width / 2.0F,
        ...
        {   
            ...
            bbox_data_val =  make_float4(
                                            bboxCenterX - bboxWidth / 2.0F,
                                            bboxCenterY - bboxHeight / 2.0F,
                                            bboxCenterX + bboxWidth / 2.0F,
                                            bboxCenterY + bboxHeight / 2.0F
                                        );
        ...
        // store to gmem
        from_float4(
                    *(reinterpret_cast<VECTOR_TYPE*>(bbox_data + 4 * index)),
                    bbox_data_val);
    }
}
    ...
    const bool packed32_nchw)
{
    ...
    decodeBBoxesOpt_kernel<T_BBOX, BS, NUM_LAYERS><<<GS, BS, 0, stream>>>(nthreads/4, code_type, variance_encoded_in_target,
...
{
    dbbFuncVec.push_back(dbbLaunchConfig(DataType::kFLOAT, 5,
                                         decodeBBoxesOpt_gpu<float, 5>));
    dbbFuncVec.push_back(dbbLaunchConfig(DataType::kFLOAT, 6, 
                                         decodeBBoxesOpt_gpu<float, 6>));
