...
__device__ __inline__ Dtype permuteConfData(
        ...
        int fast_divmod9_mul, int fast_divmod9_shr,
        bool confSigmoid,
  ...
  if (background_class_id != class_id) {
      ...
      for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
          ...
          if (offset < index_limit) {
              thread_items[i] = __float_as_int(permuteConfData<float, NUM_LAYERS>(
                                                                         index,
                                                                         ...
                                                                         fast_divmod9_mul, fast_divmod9_shr,
                                                                         confSigmoid,
                                                                         permute_conf_data));
          }
      }
    ...
    const bool packed32_nchw)
{
    ...
    top_k_kernels[kernel_index]<<<grid, block, smem_size, stream>>>((int*) (conf_scores_gpu), (int*) (conf_scores_gpu), (int*)index_array_gpu,
                                                                    ...
                                                                    fast_divmod_9_mul, fast_divmod_9_shr,
                                                                    confSigmoid,
                                                                    permute_conf_data
...
bool tkspcpInit()
{
    tkspcpFuncVec.push_back(tkspcpLaunchConfig(DataType::kFLOAT, 5,
                                           topKScoresPerClass_gpu<float, 5>));
    tkspcpFuncVec.push_back(tkspcpLaunchConfig(DataType::kFLOAT, 6,
                                           topKScoresPerClass_gpu<float, 6>));
    return true;
}
