...
DetectionOutputOpt<T>::DetectionOutputOpt(DetectionOutputParameters params,
    bool confSoftmax, bool permuteBeforeReshape, bool concatInputs, 
    int numLayers, bool supportsInt8ConfInputs)
    : param(params), mConfSoftmax(confSoftmax)
    , mPermuteBeforeReshape(permuteBeforeReshape)
    , mConcatInputs(concatInputs)
    , mNumLayers(numLayers)
    , mSupportsInt8ConfInputs(supportsInt8ConfInputs)
{
}
    ...
    int batchSize, const void* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream)
{
    ...
    /// debug retinanet, should be passed as extra parameter!!!
    //mPermuteBeforeReshape = 1;
    //param.codeType = CodeTypeSSD::CENTER_SIZE;
    //param.keepTopK = param.topK;
    // printf("mPacked32NCHW = %d\n", mPacked32NCHW);
    // printf("mPermuteBeforeReshape = %d\n", mPermuteBeforeReshape);
    // printf("param.varianceEncodedInTarget = %d\n", param.varianceEncodedInTarget);

    //printf("mConfPacked32NCHW = %d, mSupportsInt8ConfInputs = %d\n", mConfPacked32NCHW, mSupportsInt8ConfInputs);

    // for (int ii = 0; ii < mNumLayers; ii++) {
    //     printf("%f ", mInScale[ii]);
    // }
    // printf("\n");
    //mConfPacked32NCHW = false;
...
size_t DetectionOutputOpt<T>::getSerializationSize() const noexcept
{
    ...
    return sizeof(DetectionOutputParameters) + sizeof(int) * 4
        ...
        // std::vector<int> mInScale
        + sizeof(float) * mNumLayers
        // mPacked32NCHW
        + sizeof(bool)
        // mConfPacked32NCHW
        + sizeof(bool)
        // mSupportsInt8ConfInputs
        + sizeof(bool);
}
...
void DetectionOutputOpt<T>::serialize(void* buffer) const noexcept
{
    ...
    write(d, mConfPacked32NCHW);
    write(d, mSupportsInt8ConfInputs);
...
void DetectionOutputOpt<T>::configurePluginBase(const PluginTensorDesc* in, int nbInputs, const PluginTensorDesc* out, int nbOutputs)
{
    ...
    // get scale for conf scores
    for (int i = 0; i < mNumLayers; i++) {
        mInScale[i] = in[param.inputOrder[1] + i].scale;

        //printf("%f ", in[param.inputOrder[0] + i].scale);
    }
    //printf("\n");

	DEBUG_PRINTF("%d %d %d %d %d %d, %f, %f, %d %d %d %d %d %d\n", param.topK, param.numClasses, param.shareLocation, param.varianceEncodedInTarget,
                      param.backgroundLabelId, param.keepTopK, param.confidenceThreshold, param.nmsThreshold,
                      param.codeType, param.inputOrder[0], param.inputOrder[1], param.inputOrder[2], 
    ...
    for(int i = 0; i < mNumLayers; i++) {
        if (mConcatInputs) { // old path
            ...
            const auto boxInputType = boxInputs[i].type;
            const auto boxInputFormat = boxInputs[i].format;
            const auto confInputType = confInputs[i].type;
            const auto confInputFormat = confInputs[i].format;
            if(i == 0) { 
                // ASSERT(boxInputType == DataType::kFLOAT && (boxInputFormat == TensorFormat::kCHW32 
                //     || boxInputFormat == TensorFormat::kLINEAR));
                ASSERT(boxInputType == DataType::kFLOAT && (boxInputFormat == TensorFormat::kLINEAR));
                ASSERT(confInputType == DataType::kFLOAT && confInputFormat == TensorFormat::kLINEAR 
                    || confInputType == DataType::kINT8 && confInputFormat == TensorFormat::kCHW32);
                mConfPacked32NCHW = (confInputFormat == TensorFormat::kCHW32);
                mPacked32NCHW = false;
            }
            // if (!mConfPacked32NCHW) {
            //     mInScale[i] = 1.0f;
            // }
        }
...
bool DetectionOutputOpt<T>::supportsFormatCombinationBase(int pos, const PluginTensorDesc* inOut, int nbInputs, int nbOutputs) const
{
    ...
    } else if (pos >= param.inputOrder[1] && pos < param.inputOrder[1] + mNumLayers) //confInput
    {
        rtn = inOut[pos].type == DataType::kFLOAT && (inOut[pos].format == TensorFormat::kLINEAR)
           || mSupportsInt8ConfInputs && inOut[pos].type == DataType::kINT8  && (inOut[pos].format == TensorFormat::kCHW32);
...
NMSOptPluginCreator<T>::NMSOptPluginCreator()
{
    ...
    mPluginAttributes.emplace_back(PluginField("supportsInt8ConfInputs", nullptr, PluginFieldType::kINT32, 1));
...
IPluginV2* NMSOptPluginCreator<T>::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
    ...
    for (int i = 0; i < fc->nbFields; ++i)
    {
        ...
        else if (!strcmp(attrName, "supportsInt8ConfInputs"))
        {
            mSupportsInt8ConfInputs = static_cast<int>(*(static_cast<const int*>(fields[i].data)));
        }

    }

    return new T(params, mConfSoftmax, mPermuteBeforeReshape, mConcatInputs, mNumLayers, mSupportsInt8ConfInputs);
}
