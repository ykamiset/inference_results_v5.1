...
class DLRMv2Calibrator(trt.IInt8EntropyCalibrator2):
    ...
    def __init__(self,
        ...
        self.device_input_numeric = cudart.cudaMalloc(dense_input_size)
        self.device_input_cat = cudart.cudaMalloc(sparse_input_size)
    ...
    def get_batch(self, names):
        if self.current_idx < self.calib_max_batches:
            ...
            cudart.cudaMemcpy(self.device_input_numeric, dense_input, dense_input.size * dense_input.itemsize, cudart.cudaMemcpyKind.cudaMemcpyHostToDevice)
            cudart.cudaMemcpy(self.device_input_cat, sparse_input, sparse_input.size * sparse_input.itemsize, cudart.cudaMemcpyKind.cudaMemcpyHostToDevice)
    ...
    def __del__(self):
        cudart.cudaFree(self.device_input_numeric)
        cudart.cudaFree(self.device_input_cat)
