...
# https://github.com/mlcommons/training_results_v4.1/blob/main/NVIDIA/benchmarks/gnn/implementations/h200_ngc24.04_dgl/dataloading/overlap_dataloader.py
...
class PrefetchInterleaver:
    def __init__(self,
                 ...
                 features,
                 device,
                 repeat_input_after=-1,
                 use_high_priority_stream=True,
                 dataloader=None):
        _prio = -1 if use_high_priority_stream else 0
        self.sample_stream = torch.cuda.Stream(priority=_prio)
        self.embed_stream = torch.cuda.Stream(priority=_prio)
    ...
    def set_sample_loader(self, dataloader):
        self.dataloader = dataloader
        self.dataloader_len = len(self.dataloader)

    def unset_sample_loader(self):
