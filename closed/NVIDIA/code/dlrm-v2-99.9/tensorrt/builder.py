...
"""TensorRT builder implementation for DLRMv2 model.

This module provides the implementation for building TensorRT engines for the DLRMv2 (Deep Learning Recommendation Model v2).
It handles the conversion of the PyTorch DLRMv2 model to a TensorRT engine, including calibration for INT8 precision.

The module includes:
- DLRMv2_Model: PyTorch model loader and state management
- DLRMv2Arch: Architecture representation of DLRMv2 components
- DLRMv2TRTNetwork: TensorRT network builder
- DLRMv2EngineBuilder: Engine builder with calibration support
- DLRMv2CalibrateOp: Calibration operation
- DLRMv2BuilderOp: Builder operation

Example:
    >>> builder = DLRMv2EngineBuilder()
...
from code.ops.generate_engines import EngineBuilderOp, CalibrateEngineOp
...
Component = import_module("code.dlrm-v2.tensorrt.constants").DLRMv2Component
dlrm_fields = import_module("code.dlrm-v2.tensorrt.fields")

load_trt_plugin_by_network("dlrmv2")

class DLRMv2_Model:
    """PyTorch DLRMv2 model loader and state management.

    This class handles loading and managing the DLRMv2 model state, including:
    - Loading model weights and state dict
    - Managing embedding tables
    - Handling model distribution and parallelization
    - Converting embeddings to different precisions

    Args:
        model_path (os.PathLike): Path to the model weights directory
        num_embeddings_per_feature (int): Number of embeddings per feature
        embedding_dim (int): Dimension of embedding vectors
        dcn_num_layers (int): Number of DCN layers
        dcn_low_rank_dim (int): Low rank dimension for DCN
        dense_arch_layer_sizes (List[int]): Layer sizes for dense architecture
        over_arch_layer_sizes (List[int]): Layer sizes for over architecture
        load_ckpt_on_gpu (bool): Whether to load checkpoint on GPU
    """
...
class DLRMv2Arch:
    """Architecture representation of DLRMv2 components.

    This class represents the DLRMv2 architecture components:
    - Bottom MLP (DenseArch)
    - CrossNet (InteractionDCNArch)
    - Top MLP (OverArch)
    - Final linear layer

    The architecture is based on TorchRec's implementation:
    https://github.com/pytorch/torchrec/blob/main/torchrec/models/dlrm.py

    Args:
        state_dict: Model state dictionary
        bot_mlp_depth (int): Depth of bottom MLP
        crossnet_depth (int): Depth of crossnet
        top_mlp_depth (int): Depth of top MLP
    """
...
precision_to_trt = {
    Precision.FP32: trt.float32,
    Precision.FP16: trt.float16,
    Precision.INT8: trt.int8,
    Precision.INT32: trt.int32,
}
...
@bind(model_fields.input_dtype, "dense_dtype")
@bind(dlrm_fields.bot_mlp_precision)
@bind(dlrm_fields.embedding_weights_on_gpu_part)
@bind(dlrm_fields.embeddings_path)
@bind(dlrm_fields.embeddings_precision)
@bind(dlrm_fields.interaction_op_precision)
@bind(dlrm_fields.top_mlp_precision)
@bind(dlrm_fields.final_linear_precision)
class DLRMv2TRTNetwork:
    """TensorRT network builder for DLRMv2.

    This class handles the conversion of DLRMv2 model to a TensorRT network,
    including:
    - Network definition
    ...
    Args:
        network (trt.INetworkDefinition): TensorRT network definition
        batch_size (int): Batch size for the network
        verbose (bool): Enable verbose logging
        use_row_frequencies_opt (Optional[bool]): Use row frequencies optimization
        embedding_weights_on_gpu_part (float): Portion of embedding weights on GPU
        dense_dtype (Precision): Precision for dense operations
        sparse_dtype (Precision): Precision for sparse operations
        bot_mlp_precision (Precision): Precision for bottom MLP
        embeddings_precision (Precision): Precision for embeddings
        interaction_op_precision (Precision): Precision for interaction operations
        top_mlp_precision (Precision): Precision for top MLP
        final_linear_precision (Precision): Precision for final linear layer
        model_path (os.PathLike): Path to model weights
        embeddings_path (os.PathLike): Path to embedding weights
    """
    ...
    def __init__(self,
                 ...
                 batch_size: int,
                 verbose: bool = False,
                 use_row_frequencies_opt: Optional[bool] = None,
                 embedding_weights_on_gpu_part: float = 1.0,
                 dense_dtype: Precision = Precision.FP16,
                 sparse_dtype: Precision = Precision.INT32,
                 bot_mlp_precision: Precision = Precision.INT8,
                 embeddings_precision: Precision = Precision.INT8,
                 interaction_op_precision: Precision = Precision.INT8,
                 top_mlp_precision: Precision = Precision.INT8,
                 final_linear_precision: Precision = Precision.INT8,
        ...
        self.dense_dtype = precision_to_trt[dense_dtype]
        self.sparse_dtype = precision_to_trt[sparse_dtype]
        self.bot_mlp_precision = precision_to_trt[bot_mlp_precision]
        self.embeddings_precision = precision_to_trt[embeddings_precision]
        self.interaction_op_precision = precision_to_trt[interaction_op_precision]
        self.top_mlp_precision = precision_to_trt[top_mlp_precision]
        self.final_linear_precision = precision_to_trt[final_linear_precision]

        self.embedding_weights_on_gpu_part = embedding_weights_on_gpu_part
        if use_row_frequencies_opt is None:
            self.use_row_frequencies_opt = embeddings_precision != Precision.INT8
        else:
        ...
        self.mega_table_npy_file = self.embeddings_path / f'mega_table_{embeddings_precision.valstr}.npy'
...
class DLRMv2EngineBuilder(CalibratableTensorRTEngine,
                          TRTBuilder):
    """TensorRT engine builder for DLRMv2 with calibration support.

    This class extends TRTBuilder to provide DLRMv2-specific engine building
    ...
    Args:
        workspace_size (int): Workspace size for TensorRT
        component (str): Component identifier
        batch_size (int): Batch size for the engine
        calib_batch_size (int): Batch size for calibration
        calib_max_batches (int): Maximum number of calibration batches
        calib_data_map (os.PathLike): Path to calibration data map
        cache_file (os.PathLike): Path to calibration cache file
        **kwargs: Additional arguments passed to parent classes
    """

    def __init__(self,
                 workspace_size: int = 8 << 30,
                 component: str = Component.DLRMv2.valstr,
                 batch_size: int = 8192,
                 # calibration setup
        ...
        self.timing_cache_file = paths.BUILD_DIR / "cache" / f"dlrm_build_cache_{self.precision}.cache"
    ...
    def create_network(self, builder: trt.Builder):
        ...
        dlrm_network = DLRMv2TRTNetwork(base_network, self.batch_size)
    ...
    def create_builder_config(self, profiles: List[trt.IOptimizationProfile]):
        ...
        if self.use_timing_cache:
            ...
            if self.timing_cache_file.exists():
...
class DLRMv2CalibrateOp(CalibrateEngineOp):
    """Calibration operation for DLRMv2 engine.

    This class handles the calibration process for DLRMv2 engine,
    including:
    - Dataset loading
    - Network creation for calibration
    ...
    def run(self, scratch_space, dependency_outputs):
        ...
        assert len(self.engine_index.engines) == 1, "DLRM should only have 1 component enumerated"
        _L = [(c_eng, builder_cls) for c_eng, builder_cls in self.engine_index.iter_components()]
        component_engine, builder_cls = _L[0]
        batch_size = component_engine.batch_size
        builder = builder_cls(batch_size=batch_size)
        if not builder.need_calibration:
            logging.info(f"Calibration not needed for {component_engine.component}, skipping.")
            return
        ...
        def create_network(_, network):
            ...
            overrides = {"dense_dtype": Precision.FP32,
                         "bot_mlp_precision": Precision.FP32,
                         "embeddings_precision": Precision.FP32,
                         "interaction_op_precision": Precision.FP32,
                         "top_mlp_precision": Precision.FP32,
                         "final_linear_precision": Precision.FP32,
                         ...
                         "use_row_frequencies_opt": False}

            DLRMv2TRTNetwork(network,
        ...
        calibrator = Calibrator(data_loader=data_loader(), cache=builder.cache_file, batch_size=batch_size)
        ...
        with G_LOGGER.verbosity(G_LOGGER.VERBOSE), TrtRunner(load_engine) as _:
            logging.info(f'Calibration completed, cache written to: {builder.cache_file}')


class DLRMv2BuilderOp(EngineBuilderOp):
    """Builder operation for DLRMv2 engine.

    This class handles the engine building process for DLRMv2,
    including:
    - Component iteration
    - Network creation
    - Profile management
    ...
    def run(self, scratch_space, dependency_outputs):
        if self.use_mps:
            mps_scope = MPS(active_sms=active_sms)
        else:
        ...
        with mps_scope:
            for c_eng, builder_cls in self.engine_index.iter_components():
                ...
                builder(builder.batch_size,
                        self.engine_index.full_path(c_eng),
                        network=network,
                        profiles=profiles,
                        builder_config=builder_config)

                if builder.use_timing_cache:
                    builder.timing_cache_file.parent.mkdir(parents=True, exist_ok=True)

                    # save latest timing cache
                    with builder.timing_cache_file.open(mode='wb') as f:
