...
from .utilities import EmbeddingDims, PipelineConfig
...
class AbstractModel:
    def __init__(self,
        ...
        assert self.precision in [Precision.FP32,
                                  Precision.FP16,
                                  Precision.INT8,
                                  Precision.FP8], f"Invalid model precision: {self.precision}"
...
class UNetXL(AbstractModel):
    ...
    @AbstractModel.check_dims
    def get_sample_input(self, batch_size):
        dtype = torch.float16 if self.precision == Precision.FP16 else torch.float32
        latent_height = PipelineConfig.LATENT_SIZE
        latent_width = PipelineConfig.LATENT_SIZE
        return (
