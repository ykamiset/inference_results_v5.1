...
// QSL NUMA config. Each QSL may map to multiple NUMA node.
using QSLNumaConfig = std::vector<std::vector<int>>;

// QSL idx for given NUMA index
using NumaToQSLMap = std::vector<int>;
...
// Example of the format: "0-3&4&5,7&6" for 4 QSLs, 8 NUMA node system.
inline QSLNumaConfig parseQSLNumaConfig(const std::string& s)
{
    QSLNumaConfig config;
    if (!s.empty())
    {
        auto allNodes = splitString(s, "&");
        for (auto& nodes: allNodes)
        {
            auto numaNodes = parseRange(nodes);
            config.emplace_back(numaNodes);
        }
    }
    return config;
}
...
// Convert QSLNumaConfig to NumaToQSLMap for easier look-up.
inline NumaToQSLMap getNumaToQSLMap(const QSLNumaConfig& config)
{
    std::vector<int> map;
    for (int32_t qslIdx = 0; qslIdx < config.size(); qslIdx++)
    {
        for (const auto numaIdx : config[qslIdx])
        {
            if (numaIdx >= map.size())
            {
                map.resize(numaIdx + 1);
            }
            map[numaIdx] = qslIdx;
        }
    }
...
// Restrict mem allocation to specific NUMA node.
inline void bindNumaMemPolicy(const std::vector<int32_t> numaIndices, const int32_t nbNumas)
{
    unsigned long nodeMask = 0;
    for (auto& numaIdx: numaIndices)
    {
        nodeMask |= 1UL << numaIdx;
    }
    long ret = set_mempolicy(MPOL_BIND, &nodeMask, nbNumas + 1);
    CHECK(ret >= 0) << std::strerror(errno);
}
...
inline void initGlog(bool verbose)
{
    // Initialize logging
...
//! This class is a thread-safe queue data strcture that should be reused across the harnesses
template <typename T>
class SyncQueue
{
public:
    ...
    {
        std::unique_lock<std::mutex> l(mMutex);
        mCondVar.wait(l, [this] { return !mQueue.empty(); });
...
private:
    mutable std::mutex mMutex; // mutable s is required to use the mutex inside a const member function.
